...
for(sp in spp.plot) {
  N <- N.trail.series[,sp,,]
  dat.plt$v.md <- c(apply(N[,,"total"], 2, median), apply(N[,,"unexplained"], 2, median))
  dat.plt$v.lo <- c(apply(N[,,"total"], 2, function(x) quantile(x, prob = 0.1, type = 8)),
                    apply(N[,,"unexplained"], 2, function(x) quantile(x, prob = 0.1, type = 8)))
  dat.plt$v.hi <- c(apply(N[,,"total"], 2, function(x) quantile(x, prob = 0.9, type = 8)),
                    apply(N[,,"unexplained"], 2, function(x) quantile(x, prob = 0.9, type = 8)))
  names(dat.plt)[which(names(dat.plt) %in% c("v.md", "v.lo", "v.hi"))] <-
    str_c(sp, c(".md", ".lo", ".hi"))
}
dat.plt.trail <- dat.plt %>% filter(Series == "total") %>% select(-Series)
dat.plt.trail.series <- dat.plt
...
dat.plt.road <- dat.plt %>% filter(Series == "total") %>% select(-Series)
dat.plt.road.series <- dat.plt
...
dat.plt.ohv <- dat.plt %>% filter(Series == "total") %>% select(-Series)
dat.plt.ohv.series <- dat.plt
...
for(sp in spp.plot) {
  N <- N.traffic.series[,sp,]
  dat.plt$v.md <- apply(N, 2, median)
  dat.plt$v.lo <- apply(N, 2, function(x) quantile(x, prob = 0.1, type = 8))
  dat.plt$v.hi <- apply(N, 2, function(x) quantile(x, prob = 0.9, type = 8))
  names(dat.plt)[which(names(dat.plt) %in% c("v.md", "v.lo", "v.hi"))] <-
    str_c(sp, c(".md", ".lo", ".hi"))
}
dat.plt.traffic <- dat.plt
