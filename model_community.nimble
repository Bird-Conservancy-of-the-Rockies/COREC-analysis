model <<- nimbleCode({
    ...
    for(s in 1:nspp) { ## Start species loop ##
      ...
      for(t in 1:nyear) {
        dev.zeta0[s, t] ~ dnorm(0, pow(zeta0.sd.yr, -2))
        dev.theta0[s, t] ~ dnorm(0, pow(theta0.sd.yr, -2))
      ...
      for(j in 1:ngrdyrs) { # Start gridXyear observation loop
        ...
        log(a[s, j]) <- zeta0[s] + dev.zeta0[s, yearInd[j]] + inprod(zetaVec[s, 1:n.Xpp], X.pp[j, 1:n.Xpp])
        ...
        logit(p_a[s, j]) <- theta0[s] + dev.theta0[s, yearInd[j]] + inprod(thetaVec[s, 1:n.Xpa], X.pa[j, 1:n.Xpa])
        ...
        prob_n[s, j] <- pdet[s, j] * phi[s, j] * (effort[j] / 16)
        n[s, j] ~ dbin(prob_n[s, j], N[s, j])
        
        ##~~~~~~~~~~~~~~ State process occupancy model ~~~~~~~~~~~~~~##
        logit(psi[s, j]) <- alpha0[s] + dev.alpha[s, yearInd[j]] + inprod(alphaVec[s, 1:n.Xalpha], X.alpha[j, 1:n.Xalpha])
        z[s, j] ~ dbern(psi[s, j]) # Occupancy state
        
        ##~~~~~~~~~~~~~~ State process abundance model ~~~~~~~~~~~~~~##
        log(lambda[s, j]) <- beta0[s] + dev.beta[s, yearInd[j]] + inprod(betaVec[s, 1:n.Xbeta], X.beta[j, 1:n.Xbeta])
