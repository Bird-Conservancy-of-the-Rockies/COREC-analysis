...
where $\theta_{0,i}$ and $\zeta_{0,i}$ are intercepts, $\mathbf{\theta_i}$ and $\mathbf{\zeta_i}$ are coefficient vectors for covariate relationships, and $\mathbf{X_{jt}}$ are matrices of values for detectability covariates. For availability, these were unconditional traffic intensity, survey date, survey time, and squared values for date and time. For perceptibility, we only had unconditional traffic intensity as a covariate. As with ecological parameters, we modeled all intercepts and covariate coefficients as species-specific random effects. Hyper-parameter priors for detectability random effects were
